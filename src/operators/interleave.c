                    ...
                    struct csp_name_visitor *visitor)
{
    ...
    csp_process_set_nested_name(csp, process, &interleave->ps, "⫴", visitor);
}
                        ...
                        struct csp_event_visitor *visitor)
{
    ...
    /* Rules 1 and 2 */
    csp_process_set_foreach (&interleave->ps, &i) {
                             ...
                             struct csp_edge_visitor *visitor)
{
    ...
    csp_process_set_union(&ps_prime, &interleave->ps);
    /* For all P ∈ Ps */
    csp_process_set_foreach (&interleave->ps, &i) {
                                   ...
                                   struct csp_edge_visitor *visitor)
{
    ...
    csp_process_set_foreach (&interleave->ps, &i) {
        ...
        if (contains.is_present) {
            ...
            csp_process_set_remove(&ps_prime, p);
            csp_process_set_add(&ps_prime, csp->stop);
            csp_edge_visitor_call(csp, visitor, initial,
                                  csp_interleave(csp, &ps_prime));
            /* Reset Ps' back to Ps. */
            csp_process_set_remove(&ps_prime, csp->stop);
            csp_process_set_add(&ps_prime, p);
        }
    }
    csp_process_set_done(&ps_prime);
...
csp_interleave_free(struct csp *csp, struct csp_process *process)
{
    struct csp_interleave *interleave =
            container_of(process, struct csp_interleave, process);
    csp_process_set_done(&interleave->ps);
    free(interleave);
}
...
static csp_id
csp_interleave_get_id(const struct csp_process_set *ps)
{
...
csp_interleave_new(struct csp *csp, const struct csp_process_set *ps)
{
    ...
    csp_process_set_init(&interleave->ps);
    csp_process_set_union(&interleave->ps, ps);
